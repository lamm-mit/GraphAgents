...
def find_top_n_simple_paths_between_nodes(graph, nodes, n=10):
    """
    For every pair of nodes in the input list, find up to the top n shortest simple paths,
    then return a subgraph containing all nodes and edges involved in those paths.

    Parameters:
    - graph: NetworkX graph (weighted or unweighted)
    - nodes: list of nodes to consider in pairwise combinations
    - n: number of shortest simple paths to extract per node pair

    Returns:
    - A NetworkX subgraph containing the union of all top-n paths between node pairs
    """
    subgraph_nodes = set()
    subgraph_edges = set()
    found = 0
    ...
    for i in range(len(nodes)):
        for j in range(i + 1, len(nodes)):
            ...
            try:
                # Generator for paths sorted by path length
                path_generator = shortest_simple_paths(graph, source, target)

                # Take top-n paths
                top_paths = list(itertools.islice(path_generator, n))

                for k, path in enumerate(top_paths):
                    print(f"Path {k+1} between {source} and {target} (length {len(path)-1}): {path}")
    ...
    print(f"Path found ratio = {found / all_path_attempts:.2f}")

    # Build and return the subgraph from collected edges
    return graph.edge_subgraph(subgraph_edges).copy()


def find_dfs_path_subgraph_between_nodes(graph, nodes, max_depth=10):
    subgraph = set()          # Set to accumulate all nodes involved in found DFS paths
    found = 0                 # Counter for how many node pairs have a path found
    all_path = 0              # Counter for total pairs checked

    # Inner recursive DFS helper function
    def dfs(current, target, visited, depth):
        if depth > max_depth:
            return None       # Stop exploring if max depth exceeded (to limit search)
        if current == target:
            return [current]  # Found target: return path consisting of just this node
        visited.add(current)  # Mark current node as visited to avoid cycles
        for neighbor in graph.neighbors(current):   # Explore all neighbors of current
            if neighbor not in visited:              # Only proceed if neighbor unvisited
                path = dfs(neighbor, target, visited, depth + 1)  # Recursive call deeper by one
                if path:                            # If a path is found from neighbor to target
                    return [current] + path       # Append current node at front and return path
        visited.remove(current)  # Backtrack: unmark current node before returning None
        return None             # No path found from current node

    # Loop through all unique pairs (i, j) in nodes list where j > i
    for i in range(len(nodes)):
        for j in range(i + 1, len(nodes)):
            all_path += 1                # Increment total pair count
            visited = set()             # Reset visited set for this pair’s DFS search
            path = dfs(nodes[i], nodes[j], visited, 0)  # Start DFS from nodes[i] to nodes[j]
            if path:
                print(f'Path between {nodes[i]}, {nodes[j]} found as {path}')  # Print found path
                subgraph.update(path)   # Add nodes from found path into the subgraph node set
                found += 1              # Increment count of found paths
            else:
                print(f'No path between {nodes[i]}, {nodes[j]} found')  # No path found for pair

    print(f'Path found ratio = {found / all_path}')  # Print success rate for pathfinding
    return graph.subgraph(list(subgraph))  # Return the induced subgraph containing all found paths


from collections import deque

def bfs_with_semantic_stop(graph, nodes, semantic_stop_node):
    """
    For every pair of nodes in the list `nodes`, run BFS from nodes[i] to nodes[j]
    and return the first path that passes through `semantic_stop_node`.
    Returns a subgraph containing all such paths.
    """
    subgraph_nodes = set()      # Collects all nodes in successful paths
    found = 0                   # Counter for successful paths found
    all_pairs = 0               # Total number of node pairs considered
    ...
    for i in range(len(nodes)):
        for j in range(i + 1, len(nodes)):
            ...
            queue = deque([[source]])  # Start BFS queue with the source node as a path
            visited = set()
            found_path = None
            ...
            while queue:
                ...
                # Check if target is reached and the semantic stop node is in the path
                if node == target and semantic_stop_node in path:
                    found_path = path
                    print(f"Found path from {source} to {target} via {semantic_stop_node}: {path}")
                    subgraph_nodes.update(path)  # Add all nodes in the path to subgraph
                    found += 1
            ...
            if not found_path:
                print(f"No valid path from {source} to {target} via {semantic_stop_node}")

    print(f"Found {found} paths out of {all_pairs} node pairs that include '{semantic_stop_node}'")
    return graph.subgraph(subgraph_nodes)  # Return subgraph of all found paths

def collect_entities(graph): #converts graph to natural language
...
EXTRACT_PROMPT = '''
Extract as many meaningful keywords as possible into a JSON list for the given question. Only provide the JSON list. See the following:
Examples:
...
MATERIAL_KEYWORD_EXTRACTOR = '''
Convert the semicolon-delimited material properties string into a structured JSON list. 
Follow these rules:
1. Split by semicolons first
2. For property:value pairs, keep as single terms (e.g. "Tg between +100°C to -123°C" stays whole)
3. Remove any empty entries
4. Output ONLY the JSON array

Example Input: "Tg:-123:°C;O2_perm:<0.2:cm³/m²/day;cryogenic_flexibility"
Example Output: {
  "keywords": ["Tg at -123°C", "O2_permeability between <0.2:cm³/m²/day", "cryogenic_flexibility"]
}
'''

def convert_keywords_to_nodes(keyword_string, generate, node_embeddings, embedding_tokenizer, embedding_model, N_samples=5, similarity_threshold=0.9):
    # Step 1: Convert semicolon string to JSON list
    response = generate(
        system_prompt=MATERIAL_KEYWORD_EXTRACTOR,
        prompt=f'Input: ```{keyword_string}```'
    )
    ...
    try:
        ...
        if isinstance(keywords, dict) and "keywords" in keywords:
            # Handle the wrapped format if that's what's returned
            keywords = keywords["keywords"]
        print(f'Extracted material keywords: {keywords}')
    except json.JSONDecodeError as e:
        print(f'Error parsing keywords: {e}')
        return []
    
    # Step 3: Find matching nodes for each keyword
    nodes = []
    for keyword in keywords:
        ...
        if len(matched_nodes) > 0:
            nodes.extend([node[0] for node in matched_nodes])
    
    print(f'Matched nodes: {nodes}')
    return nodes
